				...
				<ul>
					<c:forEach var="myvar" items="${results}">
						<li><figure class="img-wrapper fade"> <a
							...
							<p>
								Name: ${myvar.getFname()} ${myvar.getLname()} <br>
								Email: ${myvar.getEmail()}<br>
								Address: ${myvar.getAddress()} <br>
								City: ${myvar.getCity()} <br> 
								State: ${myvar.getState()}<br>
								Zip: ${myvar.getZip()}<br> 
								Capacity: ${myvar.getCapacity()}<br>
								Profile: ${myvar.getInterests()}
							</p>
							</figure></li>
					</c:forEach>
				</ul>
